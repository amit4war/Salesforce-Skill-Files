...
- Enforce CRUD and FLS with `Security.stripInaccessible()` (preferred for field-level enforcement), `WITH SECURITY_ENFORCED` (dynamic SOQL only), or manual `isAccessible()` / `isCreateable()` checks (use only when `stripInaccessible` is structurally infeasible — document the reason).
- Prefer `Security.stripInaccessible(AccessType.READABLE, records)` as the default FLS enforcement pattern. Choose `WITH SECURITY_ENFORCED` only for dynamic SOQL. Choose manual checks only as a documented exception.
...
- Target at least 85% coverage on changed Apex classes, with meaningful assertions.
